...
var Key = new Lang.Class({
    ...
    _getExtendedKeys: function () {
        this._extended_keyboard = new St.BoxLayout({ style_class: 'key-container', //style_class: 'keyboard-layout',
                                                     vertical: false });
...
var Keyboard = new Lang.Class({
    ...
    _loadRows : function (model, level, numLevels, layout) {
        ...
        for (let i = 0; i < rows.length; ++i) {
            ...
            this._loadDefaultKeys(defaultKeysPre[level][i], keyboardRow, numLevels, row.length);
            this._addRowKeys(row, keyboardRow);
            this._loadDefaultKeys(defaultKeysPost[level][i], keyboardRow, numLevels, row.length);
    ...
    _redraw: function () {
        if (!this._enableKeyboard || !this._current_page)
            return;
...
var KeyboardController = new Lang.Class({
    ...
    getGroups: function () {
        ...
        for (let i in inputSources) {
            ...
            groups[is.index] = this._getGroupName(is);
        }
    ...
    getCurrentGroup: function () {
        return this._getGroupName(this._currentSource);
    },
